...
function ParlayJarvisTab({ fixtures, tickets, setTickets, draftLegs, setDraftLegs, budget, setBudget, budgetPct, setBudgetPct, numParlays, setNumParlays, targetOdds, setTargetOdds, marketFilter, toggleMarket, historicalRates, ensureHistoricalRates, date, onClose, engineFixtureIds, onAddLegToDraft, onFullModel, adminToken = "", jarvisBuiltTicket = null, onJarvisBuiltTicketConsumed, grmInboundCode = null, onGrmInboundConsumed, ensureFixturesForDate, goToFetchDate, crossCheckEnabled = false, unresolvableTagEnabled = false, parleyEntryPulse = 0 }) {
  ...
  // 49-FIX: independent mode state for THIS league filter instance — same
  // toggle as Live Model's, but its own on/off so switching one doesn't
  // affect the other (the two filters scope different things: Live Model's
  // fixture list vs. this builder's candidate pool).
  const [parlayLeagueFilterMode, setParlayLeagueFilterMode] = useState("include");
  ...
  const swapLeg = async (ticketId, legIndex) => {
    ...
    setTickets(prev => {
      ...
      // 49-FIX: parlayFixtures is already league-filtered (mode-aware) at its
      // own useMemo chokepoint above — this used to re-filter it AGAIN with
      // include-only logic, which would zero out results entirely in exclude
      // mode (excluded-then-required-included = impossible intersection).
      const _swapFixtures = parlayFixtures;
  ...
  const handleBuildParlay = async () => {
    ...
    if (builderMode === "jarvis") {
      ...
      const leagueNote = parlayLeagueFilter ? ` · ${parlayLeagueFilterMode === "exclude" ? "excluding" : "filtered to"} ${(parlayLeagueFilter instanceof Set ? parlayLeagueFilter : new Set([parlayLeagueFilter])).size} league(s)` : "";
                    ...
                    <TicketCard ticket={t} date={date} isJarvis={!!t.isAuto}
                      ...
                      onRemix={t.isAuto
                        ? async () => {
                            ...
                            // 49-FIX: same reasoning as the swap site above —
                            // parlayFixtures is already correctly filtered.
                            const _remixFixtures = parlayFixtures;
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:600,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"flex-end",justifyContent:"center" }}
      ...
      <div style={{ background:C.modalBg,borderRadius:"16px 16px 0 0",border:`1px solid ${C.border}`,
        width:"100%",maxWidth:480,maxHeight:"90dvh",overflowY:"auto",paddingBottom:32 }}
        onClick={e => e.stopPropagation()}>
...
function GRMProInner() {
  ...
  // "exclude" shows everything EXCEPT selected leagues. This is Live Model's
  // instance — the Custom Parley builder has its own independent copy,
  // parlayLeagueFilterMode, since <LeagueFilter> is reused by both but each
  // scopes a different thing (fixture list vs. builder candidate pool).
  ...
  // 20-FIX (root cause): .grm-header is position:fixed with a hardcoded
  // paddingTop:108 reserved for it below. That guess doesn't track the
  // header's REAL rendered height — extra badges (cached/stale, results
  // auto-refresh), text wrapping, or font-scaling can push it past 108px,
  // and whatever's first in the scrollable content (JarvisMindBox) ends up
  // permanently overlapped by the fixed header, with no amount of scrolling
  // able to reveal it. Measuring the actual header height via ResizeObserver
  // and using that (plus a small buffer) instead of a static number fixes it
  // regardless of what content the header ends up showing.
  ...
  return (
    <div ref={appScrollRef} style={{ minHeight:"100vh", background:C.bg, fontFamily:C.font }}>
      ...
      <div className="grm-desktop-shell" style={{
        ...
        paddingTop: headerHeight + 6, // +6px buffer so content never sits flush
      }}>

      {/* ── HEADER — 3-level premium shell ───────────────────────────── */}
      <div className="grm-header" ref={headerRef}>
