...
function ParlayJarvisTab({ fixtures, tickets, setTickets, draftLegs, setDraftLegs, budget, setBudget, budgetPct, setBudgetPct, numParlays, setNumParlays, targetOdds, setTargetOdds, marketFilter, toggleMarket, historicalRates, ensureHistoricalRates, date, onClose, engineFixtureIds, onAddLegToDraft, onFullModel, adminToken = "", jarvisBuiltTicket = null, onJarvisBuiltTicketConsumed }) {
  ...
  // CL-FIX: When Jarvis sends a pre-built ticket via "Book Now" / "View Full",
  // save it immediately and open it as the active ticket — user sees Book Now right away.
  useEffect(() => {
...
export default function GRMPro() {
  ...
  // CL-FIX: Pre-built ticket from Jarvis chat — passed into Parley system as active built ticket
  ...
  const handleJarvisNavigate = useCallback((dest) => {
    ...
    if (navTab === "parley" || dest.ticket) {
      ...
      // CL-FIX: If Jarvis sends a pre-built ticket, load it into the parley system
      // as an active built ticket so user can book in one tap — no draft dance.
      if (dest.ticket) {
